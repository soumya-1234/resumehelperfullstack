                        ...
                        <div class="col-12 col-md-9 col-lg-7 col-xl-6">
                            <div class="card" style="border-radius: 15px;">
                                <div class="card-body p-5">
                                    <h2 class="text-uppercase text-center mb-5" style="color:blueviolet;">Login for Recruiter</h2>
                                    ...
                                    <strong>Not registered? <router-link :to="{path: '/recrsignup' ,query:{navb:true}}">Sign Up</router-link></strong>
                                </div>
                            </div>
                        </div>
                    </div>
    ...
    methods:{
      login(){
        if(this.email==""){
          ...
          if(this.auth){
            this.$store.commit("setRecrauthentication",true);
            this.$store.commit("setNavbar",false);
            this.$store.commit("setNavbar1",false);
            this.$store.commit("setNavbar2",true);
            this.$router.replace({name: "recrhome", query:{navb:false,rname: this.studname,rid: this.studid}});
          } 
          else{
            this.boole=true
            this.boole1=true
